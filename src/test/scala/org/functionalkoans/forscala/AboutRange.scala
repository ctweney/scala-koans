...
class AboutRange extends KoanSuite {
  ...
  koan("Range are not inclusive at end of range") {
    ...
    someNumbers.head should be(0)
    someNumbers.tail should be(Range(1,10))
    someNumbers.tail.tail should be(Range(2,10))
    someNumbers.tail.tail.tail should be(Range(3,10))
    someNumbers.head should be(0)
  }
